    ...
    <section class="surface-document" aria-label="Axone Surface">
      ...
      <header class="doctrine-hero">
        <Transition name="law-fade" mode="out-in">
          <p :key="activeLaw.id" class="law-line" :aria-label="`${activeLaw.number} / ${activeLaw.title} - ${activeLaw.paraphrase}`" aria-live="polite" aria-atomic="true">
      ...
      <section class="surface-bridge" aria-label="Surface state">
        <div class="surface-gestures">
          <a href="#surface-act-stream-title">Enter the surface <span aria-hidden="true">→</span></a>
        </div>
      </section>
