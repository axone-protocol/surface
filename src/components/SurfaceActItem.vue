...
function startTyping() {
  ...
  typingTimer = window.setInterval(() => {
    ...
    typedLength.value = Math.min(fullText.value.length, typedLength.value + (nextCharacter === " " ? 2 : 1));
  }, 16);
}
...
<template>
  <article class="surface-act-item" :data-kind="act.kind">
    ...
    <p v-if="hasTypedLine('description')" class="surface-act-item-description">{{ typedLine("description") }}</p>
  </article>
</template>
